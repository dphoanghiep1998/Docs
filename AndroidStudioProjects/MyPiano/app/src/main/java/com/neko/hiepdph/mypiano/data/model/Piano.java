...
import com.neko.hiepdph.mypiano.common.MyExtensionKt;
...
public class Piano {
    ...
    private List<PianoStyleModel> list = new ArrayList();


    private void initPiano() {
        if (scaleX > 0) {
            //获取黑键和白键的高度和宽度
            Drawable blackDrawable = ContextCompat.getDrawable(context, R.drawable.bg_black_piano);
            Drawable whiteDrawable = ContextCompat.getDrawable(context, R.drawable.bg_white_piano);
            ...
            for (int i = 0; i < BLACK_PIANO_KEY_GROUPS; i++) {
                ...
                for (int j = 0; j < keys.length; j++) {
                    ...
                    if (MyExtensionKt.getConfig(context).getReverb()) {
                        keys[j].setVoiceId(getVoiceFromResources("b" + i + j + "1"), context);
                    } else {
                        keys[j].setVoiceId(getVoiceFromResources("b" + i + j), context);
                    }
                    keys[j].setPressed(false);
                    keys[j].setKeyDrawable(Objects.requireNonNull(FastUtils.INSTANCE.getStyle(context, MyExtensionKt.getConfig(context).getIndexTheme(), scaleX, scaleY)).getListBlackKeyDrawable());
//                    keys[j].setKeyDrawable(new ScaleDrawable(ContextCompat.getDrawable(context, R.drawable.bg_black_piano), Gravity.NO_GRAVITY, scaleX, scaleY).getDrawable());
            ...
            for (int i = 0; i < WHITE_PIANO_KEY_GROUPS; i++) {
                ...
                for (int j = 0; j < mKeys.length; j++) {
                    ...
                    if (MyExtensionKt.getConfig(context).getReverb()) {
                        mKeys[j].setVoiceId(getVoiceFromResources("w" + i + j + "1"), context);
                    } else {
                        mKeys[j].setVoiceId(getVoiceFromResources("w" + i + j), context);

                    }
                    mKeys[j].setPressed(false);
                    mKeys[j].setKeyDrawable(Objects.requireNonNull(FastUtils.INSTANCE.getStyle(context, MyExtensionKt.getConfig(context).getIndexTheme(), scaleX, scaleY)).getListWhiteKeyDrawable().get(count % 7));

                    count += 1;
//                    mKeys[j].setKeyDrawable(new ScaleDrawable(ContextCompat.getDrawable(context, R.drawable.bg_white_piano), Gravity.NO_GRAVITY, scaleX, scaleY).getDrawable());
