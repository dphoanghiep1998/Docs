...
import com.neko.hiepdph.mypiano.common.listener.LoadAudioMessage;
import com.neko.hiepdph.mypiano.common.listener.OnLoadAudioListener;
import com.neko.hiepdph.mypiano.data.model.Piano;
import com.neko.hiepdph.mypiano.data.model.PianoKey;
...
public class AudioUtils implements LoadAudioMessage {
    ...
    private AudioUtils(Context context, OnLoadAudioListener loadAudioListener, int maxStream) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            pool = new SoundPool.Builder().setMaxStreams(maxStream).setAudioAttributes(new AudioAttributes.Builder().setContentType(AudioAttributes.CONTENT_TYPE_MUSIC).setUsage(AudioAttributes.USAGE_MEDIA).build()).build();
        } else {
    ...
    public static AudioUtils getInstance(Context context, OnLoadAudioListener listener, int maxStream) {
            ...
            if (isLoadFinish) {
                if (key.getType() != null) {
                    service.execute(() -> {
                        switch (key.getType()) {
                            case BLACK:
                                playBlackKeyMusic(key, key.getGroup(), key.getPositionOfGroup());
                                break;
                            case WHITE:
                                playWhiteKeyMusic(key, key.getGroup(), key.getPositionOfGroup());
                                break;
                        }
                    });
                }
     ...
     */
    private void playWhiteKeyMusic(PianoKey key, int group, int positionOfGroup) {
        int position;
        if (group == 0) {
        ...
        play(key, whiteKeyMusics.get(position));
    }
     ...
     * @param positionOfGroup 组内位置
     */
    private void playBlackKeyMusic(PianoKey pianoKey, int group, int positionOfGroup) {
        int position;
        if (group == 0) {
            position = positionOfGroup;
        } else {
        ...
        play(pianoKey, blackKeyMusics.get(position));
    }

    private void play(PianoKey pianoKey, int soundId) {
        float volume = 1;
